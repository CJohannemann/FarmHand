...
-- A buyer (or anyone else you'd otherwise re-type into a "Note" every time).
-- Deliberately not an asset: it isn't stock, and asset is read unfiltered in
-- several places (the whole Inventory list, the "add to inventory" empty
-- state, a chore's "What for?") that a buyer showing up in would break.
...
create index on contact (farm_id);
